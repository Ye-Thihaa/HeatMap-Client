import { Bell, TrendingUp, TrendingDown, Sparkles } from 'lucide-react'
...
function NotificationRow({ n, onRead, tFn }: { n: HeatRiskNotification; onRead: (id: string) => void; tFn: (k: string, v?: Record<string, string>) => string }) {
  const Icon = n.direction === 'up' ? TrendingUp : TrendingDown
  ...
  return (
    ...
    >
      <div className={`mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-full ${RISK_COLORS[n.to_risk] ?? 'text-ink-600 bg-mist-100'}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-sm text-ink-600">
          {tt(
            tFn,
            'notifications.riskChanged',
            `Risk level changed from ${n.from_risk} to ${n.to_risk}`,
            { from: n.from_risk, to: n.to_risk }
          )}
          {n.current_temp_c != null && ` · ${n.current_temp_c}°C`}
        </p>
